...
function validateConfig(config) {
    for (const [key, value] of Object.entries(config)) {
        if (!Number.isSafeInteger(value))
            throw new Error(`tool-monitor: ${key} must be a whole number`);
    }
}
function validateArgs(args) {
    if (args.job_id.trim().length === 0)
    ...
    if (args.pattern.length === 0)
        throw new Error('invalid pattern: expected a non-empty regular expression');
    try {
...
function targetFinishedOutcome(targetId, outcome) {
    return {
        status: 'completed',
...
export function apply(ctx, input = {}) {
    ...
    ctx.tools.register(defineTool({
        ...
        execute(args, exec) {
            ...
            if (owner === undefined)
                throw new Error('job_monitor requires an agent-owned session');
            if (exec.signal.aborted)
                throw new Error('job_monitor start aborted');
            ...
            monitorId = ctx.jobs.start({
                ...
                run: () => {
                    ...
                    const flushNotice = () => {
                        ...
                        if (stopped || pending.length === 0 || monitorId === undefined)
                            return;
                        const lines = pending;
                        pending = [];
                    ...
                    const finish = (outcome, discardPending = false) => {
                        if (settled)
                            return;
                        settled = true;
                    ...
                    const subscription = jobs.subscribeOutput(targetId, owner, {
                        onLines(lines) {
                            ...
                            if (deliveryTimer === undefined)
                                deliveryTimer = setTimeout(flushNotice, config.batchWindowMs);
                        },
                        onDone(outcome) {
